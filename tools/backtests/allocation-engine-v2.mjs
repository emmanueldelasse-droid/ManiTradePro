...
// Caps FINS par sous-thème (v2 — nouveauté de cette PR)
// Une position porte plusieurs sous-thèmes. Le cap fin s'applique à la somme
// des poids des positions partageant chaque sous-thème (multi-tag).
const FINE_THEME_CAPS = {
  us_growth_etf: 0.30,
  mega_cap_tech: 0.25,
  software_saas: 0.25,
  ai_hypergrowth: 0.20,
...
// === MIROIR de la classification de theme-classification-v2.mjs ===
//
// Le brief de cette PR interdit de modifier theme-classification-v2.mjs (qui
// ne fait pas d'export). On duplique donc la table à dessein pour ne pas
// dépendre d'un changement d'API là-bas. À unifier dans une PR future si
// l'on accepte d'exporter CLASSIFICATION depuis le module v2.
//
// Toute évolution de la classification DOIT être propagée aux deux fichiers,
// sinon les deux moteurs diverge silencieusement. C'est de la dette technique
// volontaire et documentée.
const CLASSIFICATION = {
  ...
  // EDA software pour fondeurs semis — ajouté ici v2 car non présent
  // dans theme-classification-v2.mjs (interdit de modifier). Dette à
  // unifier dans une PR future.
