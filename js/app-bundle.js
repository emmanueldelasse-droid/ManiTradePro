...
Router.register('portefeuille', () => { renderPortefeuille(); });
  ...
  screen.innerHTML = `
    <div class="screen-header">
      ...
      <p class="screen-subtitle">Simulation et réel — mises à jour toutes les 5 min</p>
    </div>
    ...
    <section class="positions-section sim-section">
      ...
      ${simWithPnl.length > 0 ? `
        <div class="positions-summary sim-summary">
          <div class="summary-item"><span class="summary-label">P&L total</span><span class="summary-value ${Fmt.pnlClass(simTotalPnl)}">${Fmt.signedCurrency(simTotalPnl)}</span></div>
          <div class="summary-item"><span class="summary-label">Positions</span><span class="summary-value">${simWithPnl.length}</span></div>
          <div class="summary-item"><span class="summary-label">En profit</span><span class="summary-value positive">${simWithPnl.filter(p => p.pnl > 0).length}</span></div>
          <div class="summary-item"><span class="summary-label">En perte</span><span class="summary-value negative">${simWithPnl.filter(p => p.pnl < 0).length}</span></div>
        </div>
        <div class="positions-list">${simWithPnl.map(p => _renderPositionCard(p, 'sim')).join('')}</div>
      ` : `
        <div class="empty-positions">
          <div class="empty-icon">📊</div>
          <p class="empty-title">Aucune position fictive ouverte</p>
          <p class="empty-sub">Ouvrez une position depuis la fiche d'un actif</p>
          <button class="btn-primary btn-sm" data-screen="opportunities">Voir les opportunités</button>
        </div>
      `}
      ...
      ${simHistory.length > 0 ? `
        <div class="history-section">
          <button class="history-toggle" id="sim-history-toggle">
            <span>Historique simulation (${simHistory.length})</span><span class="toggle-arrow">▼</span>
          </button>
          <div class="history-list hidden" id="sim-history-list">
            ${simHistory.slice().reverse().map(t => _renderHistoryRow(t)).join('')}
          </div>
        </div>
      ` : ''}
    </section>
    ...
    <section class="positions-section real-section">
      ...
      <div class="empty-positions broker-empty">
        ...
        <p class="empty-sub">Connectez votre compte Binance dans les Paramètres.</p>
        <button class="btn-outline btn-sm" data-screen="settings">Connecter un broker</button>
      </div>
    </section>

    <div class="disclaimer-card">
      <p>⚠️ Les positions réelles engagent votre capital. ManiTradePro est un outil d'aide à la décision.</p>
    </div>`;
