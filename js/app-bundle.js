...
const RealDataClient = (() => {
  ...
  const PRICE_TTL = 60 * 1000, OHLC_TTL = 60 * 60 * 1000;
  ...
  let _fxLastFetch = 0;
  const FX_TTL = 15 * 60 * 1000;

  function _defaultCurrencyForSymbol(symbol) {
    if (['MC','TTE','BNP','AIR','RMS','OR','SAN'].includes(symbol)) return 'EUR';
    if (['EURUSD','GBPUSD','USDJPY','USDCHF','AUDUSD'].includes(symbol)) return 'FX';
    return 'USD';
  }

  function _requiresEurConversion(symbol, rawCurrency) {
    const c = String(rawCurrency || _defaultCurrencyForSymbol(symbol) || '').toUpperCase();
    return c === 'USD';
  }

  async function refreshFxRate(force = false) {
    const now = Date.now();
    const current = Number.isFinite(window.__eurUsdRate) && window.__eurUsdRate > 0 ? window.__eurUsdRate : null;
    if (!force && current && (now - _fxLastFetch) < FX_TTL) return current;

    try {
      const r = await _fetchWithProxy('https://query1.finance.yahoo.com/v8/finance/chart/EURUSD=X?interval=1m&range=1d');
      if (r) {
        const d = await r.json();
        const result = d?.chart?.result?.[0];
        const meta = result?.meta || {};
        const closeSeries = result?.indicators?.quote?.[0]?.close || [];
        const lastClose = [...closeSeries].reverse().find(v => Number.isFinite(v) && v > 0) || null;
        const rate = Number.isFinite(meta?.regularMarketPrice) && meta.regularMarketPrice > 0 ? meta.regularMarketPrice : lastClose;
    ...
    try {
      const eurRes = await fetch('https://aged-bar-257a.emmanueldelasse.workers.dev/binance/api/v3/ticker/price?symbol=EURUSDT', { signal: AbortSignal.timeout(5000) });
      if (eurRes.ok) {
  ...
  async function _getYahooBatchPrices(symbols) {
    const validSymbols = (symbols || []).filter(s => YAHOO_TICKERS[s]);
    if (!validSymbols.length) return {};
    const uniqueSymbols = [...new Set(validSymbols)];
    const tickerToSymbol = Object.fromEntries(uniqueSymbols.map(s => [YAHOO_TICKERS[s], s]));

    try {
      const r = await _fetchWithProxy('https://query1.finance.yahoo.com/v7/finance/quote?symbols=' + encodeURIComponent(uniqueSymbols.map(s => YAHOO_TICKERS[s]).join(',')));
      if (!r) return {};
      const d = await r.json();
      const results = d?.quoteResponse?.result || [];
      const fxRate = await refreshFxRate(false);
      const out = {};

      for (const q of results) {
        const symbol = tickerToSymbol[q?.symbol];
        if (!symbol) continue;
        const rawPrice = Number.isFinite(q?.regularMarketPrice) ? q.regularMarketPrice : null;
        if (!rawPrice || rawPrice <= 0) continue;
        const needsFx = _requiresEurConversion(symbol, q?.currency);
        const finalPrice = needsFx ? ((fxRate && fxRate > 0) ? rawPrice / fxRate : null) : rawPrice;
        ...
        out[symbol] = {
          price: finalPrice,
          change24h: prev && prev > 0 ? ((rawPrice - prev) / prev) * 100 : null,
          volume24h: Number.isFinite(q?.regularMarketVolume) ? q.regularMarketVolume : null,
          source: 'Yahoo Finance',
          currency: needsFx ? 'EUR' : (q?.currency || _defaultCurrencyForSymbol(symbol)),
        };
      }
  ...
  async function _getYahooPrice(symbol) {
    ...
    try {
      const r = await _fetchWithProxy(`https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?interval=1m&range=1d`);
      if (!r) return null;
      const d = await r.json();
      const result = d?.chart?.result?.[0];
      const meta = result?.meta;
      ...
      const lastClose = [...closes].reverse().find(v => Number.isFinite(v) && v > 0) || null;
      const rawPrice = Number.isFinite(meta.regularMarketPrice) && meta.regularMarketPrice > 0 ? meta.regularMarketPrice : lastClose;
      if (!Number.isFinite(rawPrice) || rawPrice <= 0) return null;
      const prev = Number.isFinite(meta.previousClose) && meta.previousClose > 0 ? meta.previousClose : (Number.isFinite(meta.chartPreviousClose) ? meta.chartPreviousClose : null);
      const needsFx = _requiresEurConversion(symbol, meta.currency);
      const eurRate = needsFx ? await refreshFxRate(false) : null;
      const finalPrice = needsFx ? ((eurRate && eurRate > 0) ? rawPrice / eurRate : null) : rawPrice;
      if (!Number.isFinite(finalPrice) || finalPrice <= 0) return null;
      const res = {
        price: finalPrice,
        change24h: prev ? ((rawPrice - prev) / prev) * 100 : null,
        volume24h: Number.isFinite(meta.regularMarketVolume) ? meta.regularMarketVolume : null,
        source: 'Yahoo Finance',
        currency: needsFx ? 'EUR' : (meta.currency || _defaultCurrencyForSymbol(symbol)),
      };
      _cacheSet(ck, res); return res;
    } catch(e) { return null; }
  }
  ...
  async function _getYahooOHLC(symbol) {
    ...
    try {
      ...
      const needsFxOHLC = _requiresEurConversion(symbol, result?.meta?.currency);
      const eurRateOHLC = needsFxOHLC ? await refreshFxRate(false) : null;
      if (needsFxOHLC) {
        if (!eurRateOHLC || eurRateOHLC <= 0) return null;
  ...
  function supportsSymbol(symbol) {
    return !!(COINGECKO_IDS[symbol] || YAHOO_TICKERS[symbol]);
  }

  async function getPrice(symbol) {
    const ck = 'price_' + symbol; const cached = _cacheGet(ck); if (cached) return cached;
    if (!supportsSymbol(symbol)) return null;

    let res = null;

    // Crypto: Binance d'abord, puis CoinGecko, puis Yahoo en dernier recours
    if (COINGECKO_IDS[symbol] && BinanceClient) {
      res = await BinanceClient.getPrice(symbol);
      if (res?.price) { _cacheSet(ck, res); return res; }
    }

    if (COINGECKO_IDS[symbol]) {
      res = await _getCoinGeckoPrice(symbol);
      if (res?.price) { _cacheSet(ck, res); return res; }
    }

    // Tous les actifs Yahoo-supported doivent venir de Yahoo uniquement pour garder une source cohérente
    if (YAHOO_TICKERS[symbol]) {
      res = await _getYahooPrice(symbol);
      if (res?.price) { _cacheSet(ck, res); return res; }
      return null;
    }

    if (TwelveDataClient?.getPrice) {
      res = await TwelveDataClient.getPrice(symbol);
      if (res?.price) { _cacheSet(ck, res); return res; }
    }
  ...
  async function getOHLC(symbol) {
    ...
    if (COINGECKO_IDS[symbol]) {
      ...
      if (candles?.length >= 20) { _cacheSet(ck, candles); if (!window.__ohlcCache) window.__ohlcCache = {}; window.__ohlcCache[symbol] = candles; return candles; }
    }
    ...
    if (TwelveDataClient?.getTimeSeries) {
  ...
  async function refreshAllPrices(watchlist) {
    ...
    const filtered = watchlist.filter(a => supportsSymbol(a.symbol));
    const sorted = [
      ...filtered.filter(a => openSymbols.has(a.symbol)),
      ...filtered.filter(a => !openSymbols.has(a.symbol)),
    ];

    await refreshFxRate(false);

    const cryptoSymbols = sorted.map(a => a.symbol).filter(s => COINGECKO_IDS[s]);
    const yahooSymbols = sorted.map(a => a.symbol).filter(s => !COINGECKO_IDS[s] && YAHOO_TICKERS[s]);
    ...
    for (let i = 0; i < yahooSymbols.length; i += 20) {
      ...
      const quotes = await _getYahooBatchPrices(batch);
      for (const symbol of batch) {
        const q = quotes[symbol] || null;
        if (q?.price) {
          window.__prices[symbol] = q.price;
          _cacheSet('price_' + symbol, q);
          _cacheSet('yf_price_' + symbol, q);
        }
      }
      await new Promise(r => setTimeout(r, 120));
    }
  ...
  return { getPrice, getOHLC, refreshAllPrices, supportsSymbol, refreshFxRate };
})();
    ...
    try {
      if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
        navigator.serviceWorker.ready.then(reg => {
          reg.showNotification(title, { body, icon: '/ManiTradePro/icons/icon-192.png', badge: '/ManiTradePro/icons/icon-192.png', tag, renotify: true, vibrate: [200, 100, 200] });
        }).catch(() => {});
      } else {
        new Notification(title, { body, icon: '/ManiTradePro/icons/icon-192.png', tag });
      }
      ...
      if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
        try {
          new Notification('ManiTradePro — Signal optimal détecté', {
            body: msg, icon: '/ManiTradePro/icons/icon-192.png', tag: key,
          });
        } catch(e) {}
      }
...
async function boot() {
  ...
  try {
    const rate = await RealDataClient.refreshFxRate(true);
    if (rate) {
  ...
  // 8bis. Nettoyage des symboles custom non supportés
  try {
    const customRaw = JSON.parse(localStorage.getItem('mtp_custom_watchlist') || '[]');
    const customFiltered = customRaw.filter(a => RealDataClient.supportsSymbol(a.symbol));
    if (customFiltered.length !== customRaw.length) {
      localStorage.setItem('mtp_custom_watchlist', JSON.stringify(customFiltered));
      console.warn('[Watchlist] Symboles custom non supportés supprimés:', customRaw.filter(a => !RealDataClient.supportsSymbol(a.symbol)).map(a => a.symbol).join(', '));
    }
  } catch(e) {}
  ...
  // Auto-add trending désactivé pour éviter d'ajouter des symboles non supportés et surcharger les APIs
  ...
  // Première analyse multi-timeframe différée pour éviter les rafales API au démarrage
