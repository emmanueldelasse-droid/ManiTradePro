...
function buildWorkerPlan(base, regime = null) {
  ...
  if (structuredPlan) {
    const exploitabilityScore = clamp(Math.round(
      0.42 * Number(score || 0) +
      0.24 * Number(breakdown.entryQuality || 0) +
      0.20 * Number(breakdown.risk || 0) +
      0.08 * 70 +  // plan sur niveaux réels = bonus confirmations
      0.06 * Number(breakdown.regime || 0)
    ), 0, 100);
    ...
    return {
      ...
      finalScore: score,
      exploitabilityScore,
      setupStatus: "Setup confirme",
      confirmationCount: 5,
      confirmationLabel: "forte",
      trendLabel: direction === "long" ? "tendance haussiere" : direction === "short" ? "tendance baissiere" : "tendance neutre",
      waitFor: "rien de special",
      timing: "bon",
      safety: "elevee",
      aiSummary: `Configuration ${structuredPlan.setupType} détectée et validée en régime ${regime?.regime || "inconnu"}.`,
  ...
  const hasMajorBlocker = blockerFlags.some(f => ["data_quality_low","risk_too_high","entry_too_late","trend_conflict"].includes(f));

  const scoreGateOk = direction === "long" ? Number(score) >= 76 : direction === "short" ? Number(score) <= 26 : false;
  ...
  const exploitabilityReady = hardFilters.passed && !hasMajorBlocker && scoreGateOk &&
    entryQuality >= 64 && riskQuality >= 54 && exploitabilityScore >= 72 &&
    (direction === "long" || direction === "short");

  let decision = "Pas de trade";
  if (exploitabilityReady) decision = "Trade propose";
  else if ((direction === "long" && Number(score) >= 62) || (direction === "short" && Number(score) <= 38)) decision = "A surveiller";
  ...
  const baseStopPct = assetClass === "crypto" ? 0.05 : 0.04;
  ...
  const rrBase = entryQuality >= 65 && riskQuality >= 58 ? 2.1 : 1.8;
  ...
  return {
    ...
    trendLabel,
    waitFor: decision === "Trade propose" ? "rien de special" : "une configuration plus nette",
    side,
    entry,
    stopLoss,
    takeProfit,
    ...
    safety: score != null && score >= 72 ? "elevee" : score != null && score >= 58 ? "moyenne" : "faible",
    reason: decision === "Trade propose" ? "Setup propre, timing exploitable." : "Pas encore de configuration assez nette.",
    refusalReason: tradeNow ? null : "Signal insuffisant pour proposer un trade.",
    aiSummary: tradeNow ? "Le moteur voit un setup exploitable." : "Le moteur préfère attendre.",
    aiContext: [],
    blockerFlags,
    blockers: blockerFlags.map(f => f.replace(/_/g, " ")),
...
function buildUnavailablePayload(symbol, message = "Source temporairement indisponible") {
  ...
  return {
    ...
    plan: {
      finalScore: null, decision: "Indisponible", trendLabel: reason,
...
function buildStablePayload(symbol, quote, candles, scored, regime = null) {
  ...
  return {
    ...
    officialScore: Number.isFinite(Number(plan?.exploitabilityScore)) ? Number(plan.exploitabilityScore) : null,
...
function toOpportunityRow(payload) {
  return {
    ...
    officialScore: Number.isFinite(Number(payload?.officialScore)) ? Number(payload.officialScore) : (payload?.plan?.exploitabilityScore ?? null),
...
function isTrainingCandidateAllowed(row, settings, openRows) {
  ...
  // Scores
  if (Number(row.plan?.exploitabilityScore || 0) < Number(settings.min_actionability_score || 72)) return false;
  if (Number(row.plan?.finalScore || row.score || 0) < Number(settings.min_dossier_score || 74)) return false;
...
function buildAiContextInput(symbol, payload, newsItems=[], articleContents=[]) {
  ...
  return {
    ...
    engineContext: { score: Number(payload?.score??null), decision: String(plan?.decision||payload?.decision||""), setupStatus: String(plan?.setupStatus||""), tradeNow: !!plan?.tradeNow, trendLabel: String(plan?.trendLabel||payload?.trendLabel||""), setupType: String(plan?.setupType||payload?.setupType||""), entryQuality: Number(plan?.entryQuality??payload?.breakdown?.entryQuality??null), riskQuality: Number(plan?.riskQuality??payload?.breakdown?.risk??null), confirmationCount: Number(plan?.confirmationCount??0), blockers: Array.isArray(plan?.blockers)?plan.blockers.slice(0,10):[] },
