...
(() => {
  ...
  async function loadDashboard() {
    try {
      ...
      state.opportunities = Array.isArray(opp.data) ? opp.data : [];
  ...
  async function loadOpportunities() {
    ...
    try {
      ...
      state.opportunities = Array.isArray(result.data) ? result.data : [];
      applyFilter();
      state.error = null;
    } catch (e) {
      state.error = e.message || "Chargement impossible";
  ...
  function navigate(route, symbol=null) {
    ...
    if (route === "opportunities" && !state.opportunities.length) loadOpportunities();
    if (route === "asset-detail" && symbol) loadDetail(symbol);
    render();
  }
