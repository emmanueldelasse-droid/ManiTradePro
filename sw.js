const CACHE_NAME = 'manitradepro-v2.2';
...
self.addEventListener('fetch', e => { if (!e.request.url.includes('github.io')) return; e.respondWith(fetch(e.request).then(r => { caches.open(CACHE_NAME).then(c => c.put(e.request, r.clone())); return r; }).catch(() => caches.match(e.request))); });
