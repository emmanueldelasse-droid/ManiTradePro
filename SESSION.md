...
- **Correctifs post-déploiement (audit créateur, branche)** :
  ...
  - Cockpit V2 (évolution suivi) : `/api/v2/positions` enrichi de prix réels actuels (batch Binance + batch EODHD temps réel, Twelve/Yahoo en repli, cache KV 3 min, `currentPrice:null` si aucune source — jamais inventé) + `latentPnl/latentPnlPct/currentValue` via `computePnl` (moteur non modifié). Front : résumé positions (gagnantes/perdantes/latent/engagé/exposition), barre stop→prix→objectif avec renforcement près des bornes, durée réelle, cartes Portefeuille (equity/disponible/engagé/latent/réalisé), stats globales étendues (espérance, gain/perte moyens, drawdown en %), Top 10 actifs gagnants/perdants (réalisé+latent). `sw.js` V2 → v2-4.
